...
export const Avatar = ({
    ...
    // 🔥 Stable background color generator
    const getBgColor = (str: string) => {
        const colors = [
            'bg-[var(--color-primary)]/20 text-[var(--color-primary)]',
            'bg-blue-100 text-blue-700',
    ...
    const content = (
        <div
            className={cn(
                'relative inline-flex items-center justify-center overflow-hidden rounded-2xl border border-gray-100',
                sizeClasses[size],
                className
            )}
        >
    ...
    if (handle) {
        return (
            <Link href={`/u/${handle}`} className="cursor-pointer hover:opacity-90 transition-opacity">
                {content}
            </Link>
        );
    }
